...
def objective(opt_vars,
    ...
    max_iterations = 100
    pose = sapien.Pose(p=opt_pose[:3], q=T.euler2quat(opt_pose[3:]))

    action = transform_goal_to_wrt_base(np.concatenate([opt_pose[:3], T.euler2quat(opt_pose[3:])]))
    iter, error, target_qpos = solve_ik(ik_solver, action)
