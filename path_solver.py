...
def objective(opt_vars,
    ...
    for control_point_homo in control_points_homo:
        rot = control_point_homo[:3, :3]
        tran = control_point_homo[3, :3]
        quat = mat2quat(rot)
        iter, error, target_qpos = solve_ik(ik_solver, transform_goal_to_wrt_base(np.concatenate([tran, quat])))

        success = iter is not None

        if success:
            ik_cost += 20.0 * (iter / max_iterations)
            debug_dict['ik_feasible'].append(success)
            debug_dict['ik_pos_error'].append(error)
        else:
            ik_cost += 20.0
            debug_dict['ik_feasible'].append(success)
            debug_dict['ik_pos_error'].append(0)

        if success:
            reset_reg = np.linalg.norm(target_qpos[-1][:-1] - reset_joint_pos[:-1])
